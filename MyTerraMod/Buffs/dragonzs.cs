...
{
    public class dragonzs : ModBuff
    {
        public override void SetDefaults()
        {
            // 设置buff名字和描述
            DisplayName.SetDefault("再生");
            Description.SetDefault("大幅提高生命再生速度，受到的伤害不会超过10，生命值小于一般时生效");
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                Dust dust = Dust.NewDustDirect(player.Center, player.width, 10,
                    MyDustId.RedBlood, 0, -2f, 100, Color.White, 1.15f);
                dust.noGravity = true;
            }
        }
    }
