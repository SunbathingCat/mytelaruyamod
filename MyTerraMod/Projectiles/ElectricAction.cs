...
{
    public class ElectricAction : ModProjectile
    {
        ...
        {
            // 发出红光
            Lighting.AddLight(projectile.position, 0.0f, 0.0f, 0.1f);

            // 线性粒子效果
            for (int i = 0; i < 3; i++)
            {
                Dust d = Dust.NewDustDirect(projectile.position, projectile.width, projectile.height, MyDustId.BlueWhiteBubble, 0, 0, 100, Color.Red, 1f);
