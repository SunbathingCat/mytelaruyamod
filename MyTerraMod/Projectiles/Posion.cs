...
{
    public class Posion : ModProjectile
    {
        float baseRot = 0.0f;
        int t = 0;
            ...
            if (target != null && t % 10 < 1)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Projectile.NewProjectile(projectile.Center + projectile.velocity * 4f,
                                toTarget, mod.ProjectileType("ElectricPosion"), projectile.damage, 1f, projectile.owner, target.whoAmI);
                }
            }
        }
        public override void PostAI()
        {
            Lighting.AddLight(projectile.position, 1.0f, 0f, 1.0f);
